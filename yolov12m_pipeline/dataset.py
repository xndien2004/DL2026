...
from .config import BATCH_SIZE, WORK_DIR


class DrillData(torch.utils.data.Dataset):
    """One entry per unique image; bboxes are aggregated from the dataframe rows."""
...
def make_dataset(df: pd.DataFrame, split: str, work_dir=WORK_DIR) -> DrillData:
...
def build_dataloaders(train_df: pd.DataFrame, valid_df: pd.DataFrame, test_df: pd.DataFrame,
                      ...
                      work_dir=WORK_DIR) -> dict:
    """Returns dict with datasets + dataloaders for train/valid/test."""
    train_dataset = make_dataset(train_df, "train", work_dir=work_dir)
    valid_dataset = make_dataset(valid_df, "valid", work_dir=work_dir)
